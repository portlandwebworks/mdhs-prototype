...
module.exports = function() {
  ...
  this.Given('I am on the "$page" page', function(pageUrl, next) {
    ...
    browser.get(baseUrl+page.url);
    browser.sleep(3000).then(next);
  });
  ...
  this.When('I click the "$target" class in the "$parent" class', function(target, parent, next) {
  ...
  this.When(/^I click on "([^"]*)" in the dropdown "([^"]*)" in row "([^"]*)" of repeater "([^"]*)"(?: in parent "([^"]*)")?$/, function(option, dropdown, row, itemInRepeater, parent, next){
    var repeaterSelector = by.repeater(itemInRepeater);
    if (parent == undefined) {
      element(repeaterSelector.row(row)).element(by.model(dropdown)).element(by.cssContainingText('option', option)).click().then(next);
    } else {
      element(repeaterSelector.row(row)).element(by.css('.'+parent)).element(by.model(dropdown)).element(by.cssContainingText('option', option)).click().then(next);;
    };
  });
  ...
  this.Then('I should be on the external page "$pageName"', function (pageName, next){
    browser.getCurrentUrl().then(function(url){
      expect(url).to.equal(pageName);
      next();
    })
  })
  ...
  this.Then(/^the value of attribute "([^"]*)" of class "([^"]*)" should be "([^"]*)"(?: in parent "([^"]*)")?$/, function(attrName, cName, val, parent, next){
    if (parent == undefined){
      element(by.css('.'+cName)).getAttribute(attrName).then(function(result){
        expect(result).to.contain(val);
        next();
      });
    } else {
      element(by.css('.'+parent)).element(by.css('.'+cName)).getAttribute(attrName).then(function(result){
        expect(result).to.contain(val);
        next();
      });
    }
